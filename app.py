...
MEN_PRICES_MSG = (
    ...
    "🔹 الحمام التركي — 850 جنيه\n"
    "🔹 الحمام المغربي — 900 جنيه\n"
    "🔹 باكيدج حمام زمان — 1500 جنيه\n"
    "🔹 حمام العريس VIP — 1800 جنيه\n"
    "🔹 مساج منفرد: 30 دقيقة = 350 جنيه | 60 دقيقة = 650 جنيه\n\n"
...
def build_reply(message_text: str, sender_name: str = None):
    ...
    if intent == "men_prices":
        return MEN_PRICES_MSG
    if intent == "women_prices":
        return WOMEN_PRICES_MSG
    if intent == "address":
        return ADDRESS_MSG
    if intent == "hours":
        return HOURS_MSG
...
def handle_webhook():
    ...
    for entry in data.get("entry", []):
        for event in entry.get("messaging", []):
            ...
            reply_text = build_reply(text, sender_name)
